# Word2Vec 标准高速化实现 (学习教程与纯 NumPy 实现)
...
下图清晰描绘了以单词 `market` 为中心词、单侧窗口为 6 (共 12 个上下文词) 时的前向数据流向：

![CBOW计算全流程图解](assets/cbow_simulation_flow.png)

此外，以下交互式 Mermaid 流程图完整展现了数据张量形状的逐层蜕变：
...
├── assets/                 # 架构图解与可视化资源目录
│   ├── cbow_simulation_flow.png       # CBOW 窗口为6数据流高清解析图
│   ├── word_embeddings_2d.png         # 词向量空间 2D 语义流形聚类散点图
│   └── word_embeddings_interactive.html # 交互式 HTML5 浏览器散点图
├── data/                   # 存放 PTB 语料文件 (自动生成)
├── weights/                # 存放训练保存的模型权重 (.pkl)
└── tests/
...
本项目提供了专用的对比基准脚本 [`compare_models.py`](file:///F:/vsCode/mycode/新建文件夹/word2vec的标准高速化实现/compare_models.py)，可在**完全相同的数据切片与参数条件**下，直接运行两者的横向对比实验：
...
Word2Vec 最具魅力的特性是高维稠密空间中的“线性平移代数特性”。本项目提供了一个功能完备的交互式终端计算器 [`word_math_cli.py`](file:///F:/vsCode/mycode/新建文件夹/word2vec的标准高速化实现/word_math_cli.py)：
...
# 2. 或在命令行直接求解算式
python word_math_cli.py "take - took + go"
python word_math_cli.py "bank + market"
```

#### 交互式终端运行效果展示：
```text
VectorCalc >>> he - his + she
----------------------------------------------------------------------
  [*] 算式求解: he - his + she
----------------------------------------------------------------------
  Rank  1 | 预测词: her              | 相似度: +0.6481 [##############--------]
  Rank  2 | 预测词: herself          | 相似度: +0.5920 [############----------]
  Rank  3 | 预测词: woman            | 相似度: +0.5113 [###########-----------]
----------------------------------------------------------------------
```
...
为了直观验证神经网络是否真的学习到了语言的语义分布规律，本项目提供了双模可视化工具 [`visualize.py`](file:///F:/vsCode/mycode/新建文件夹/word2vec的标准高速化实现/visualize.py)：
...
![Word2Vec 2D 语义流形可视化](assets/word_embeddings_2d.png)
